...
```text
rule-based closed-loop structure recognition layer
```

Claim:

Chronos-S0 can read experiment diagnostics, recommend which physical language
family should enter VPSL validation next, and emit that recommendation beside
experiment outputs as `s0_recommendation.csv`.
...
S0 v0.4 preserves the K3.2D lesson:
...
S0 v0.4 includes summary adapters, a CLI, and an experiment emit helper:
...
The K3.2D experiment now calls this helper after writing
`k3_2d_0_summary.csv`, closing the first experiment-summary -> diagnostics ->
recommendation loop.

## Never-Certify Boundary

S0 v0.4 structurally rejects certifying actions at recommendation construction
time. Valid actions are only:

- `continue`
- `archive`
- `do_not_promote`

S0 does not output `CERTIFIED`, `VALIDATED`, or `PROVED`.

## Non-Claims

S0 v0.4 does not certify structures.
