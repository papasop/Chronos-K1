...
Y.Y.N. Li
Working draft v0.2 — May 2026 (post-reproduction precision update)

CHANGE LOG (v0.1 → v0.2)
========================
Four revisions, all driven by a full 3-seed reproduction run (Colab T4, 11.4 min,
paper_reproduce_all_standalone.py). All other text is unchanged.

  R1  Abstract + Section 6.5 + Section 8: replace "OOD AUC ≈ 0.45 < chance" claim
      with measured value 0.554, pivot the framing onto 0/24 strong sign-split and
      sgnAcc=0.500 (chance). The headline finding is unchanged — calibration
      destroys the sign-aware encoding — but the literal "below chance" claim is
      not supported by the rerun.
  R2  Section 4.2: insert one paragraph quantifying the ρ=0.3 result and its
      relationship to the lag-1 baseline (78.0% — already in Appendix C, now
      cross-referenced). The 3/3 direction-match at reduced signal is paper-
      strengthening evidence, previously underemphasized.
  R3  Section 5.2: replace "unstable: AUC variable" with the explicit per-seed
      range (0.001 / 0.009 / 1.000) and characterize the three solutions
      (two same-side inverted, one canonical). The ±std framing in v0.1
      understated the bimodality.
  R4  Section 4.3: tighten the markov2 paragraph with the exact mq[c0] value
      from seed 44 (the third Lorentzian seed actually does direction-match;
      v0.1 listed only seeds 42 and 43 as same-side, which is consistent with
      1/3 match, but the v0.1 wording implied seeds 42 and 43 only. The
      revision states all three explicitly.)

No reordering, no claim retraction beyond R1's "below chance" → "near chance".
Numbers in tables that already match GPU run within rounding are left as-is so
the version-to-version diff stays minimal.

================================================================================

Abstract
The Realizability theorem (Li, in prep.) shows that a small set of axioms about causal structure—reachability, energy, and time orientation—implies that the cost function on displacements must carry a Lorentzian (indefinite) signature. We ask whether neural networks trained on causally structured data spontaneously develop a representation consistent with this prediction. We construct a minimal neural architecture exposing the Lorentzian invariant m_q := ‖h_s‖² − ‖h_t‖² to the classifier, and train it to distinguish causally structured sequences from causally trivial ones. Across two backbones (a Lorentzian backbone with split normalization and a standard Euclidean backbone), four causal task structures (AR(1) vs IID, AR(2) vs AR(1) at matched lag-1, Granger x→y, do(x) intervention), three random seeds, and twenty-four out-of-distribution shifts on AR(1), the trained model consistently places the more causally structured class in the timelike region (m_q < 0) and the less causally structured class in the spacelike region (m_q > 0)—the direction predicted by the axioms. The cross-backbone, cross-structure consistency is striking: 18/18 in-distribution evaluations on independent-vs-dependent task structures align with the predicted direction, with zero inverted sign-splits across all in-distribution and OOD evaluations under natural training. We identify the technical key to obtaining this emergent sign-split—a head architecture (MqMLPHead) that gives the classifier explicit access to m_q—and show that the original bilinear inner-product head fails entirely (0/3) due to a structural algebra-geometry mismatch between bilinear heads and the quadratic invariant. Under sample-level OOD metrics, the two backbones are essentially indistinguishable (mq-AUC 0.901 vs 0.898; strong sign-split 11/24 vs 11/24), reaching the same generalization performance through what turn out to be different mechanisms: the Lorentzian backbone produces bounded, light-cone-scale encodings (|m_q| ≈ 5) while the Euclidean backbone produces unbounded magnitude separation (|m_q| ≈ 60). [REVISED R1] A calibration test forcing the Lorentzian backbone to operate at Euclidean's scale destroys its sign-aware encoding entirely (0/24 strong sign-split, sample-level sign accuracy collapses to chance, OOD mq-AUC drops from 0.904 to 0.554), demonstrating that the two backbones' sign-split solutions are not interchangeable by scale adjustment and that the Lorentzian encoding is structurally tied to its natural scale. We discuss the implications for physics-inspired inductive biases in deep learning and for the interpretation of the Realizability axioms as informational rather than purely physical statements.
...
[REVISED R2 — added paragraph]
We highlight one feature of this result that is easy to miss in the table. At ρ = 0.9 the model has a near-saturated learning signal (test accuracy 99.7%, well above the lag-1 baseline of 98.7%); the m_q gap of +11 is achieved in a regime where almost every batch fits cleanly. At ρ = 0.3 the model operates well below saturation: test accuracy (71%) is below the lag-1 baseline (78%), the gap shrinks to +2.6, and individual sample sign accuracy on the test set is only ~0.69 — many individual samples land near zero or on the wrong side. Yet the per-class means are still on opposite sides of the light-cone in 3/3 seeds. The sign-split organization survives a regime where the model has clearly not learned the lag-1 feature optimally. This is consistent with our reading of the convention as a representational structural property: it is what the head settles into even before the discriminative feature has been fully exploited, not a consequence of the model having a saturated decision surface.
[end R2]
...
[REVISED R4 — replaces the "Third" paragraph]
Third, the depth task (markov2) is the harder case for the Lorentzian backbone, with 1/3 direction-match (vs Euclidean's 3/3). Per-seed final mq[c0] values for the Lorentzian backbone were +0.39 (seed 42), +1.33 (seed 43), and −0.28 (seed 44); only seed 44 crosses zero, hence the 1/3 count. The two same-side seeds preserve mq-AUC at 0.93 and 0.92 — the informational separation between classes is intact — but mean m_q on class 0 fails to cross zero. The Euclidean backbone, with its much larger natural |m_q| scale (~25 here vs ~4 for Lorentzian), pushes mq[c0] firmly negative on all three seeds (−18, −10, −19). The asymmetry is therefore not in the existence of class-discriminative information (mq-AUC is comparable, 0.93 Lorentzian vs 0.95 Euclidean) but in whether the encoding clears the literal m_q = 0 threshold. We read this as an informative limitation rather than a failure of direction-matching: when both classes are causally structured (class 1 in markov2 is AR(1) with ρ ≈ 0.65, not i.i.d.), the bounded |m_q| ≈ 4 range available to the Lorentzian backbone has limited capacity to encode the additional depth-of-dependence ordering on top of placing samples on the correct side of the light-cone. We return to this in Section 7.4. Crucially, 0/6 markov2 runs invert the convention — the model's failure mode is to place class 0 near zero, not on the wrong side.
[end R4]
...
[REVISED R3 — replaces the post-table paragraph]
1/3 sign-split. The instability is bimodal rather than just noisy: mq-AUC values are 0.009 (seed 42), 0.001 (seed 43), and 1.000 (seed 44) — there is no seed where the head settles into a partial or intermediate solution. Two seeds converge to a "same-side, magnitude-inverted" configuration with both class means spacelike (mq[c0] ≈ +8.9, mq[c1] ≈ +5.5, with class 0 the larger of the two — opposite to the canonical direction); one seed converges to the canonical opposite-sides configuration (mq[c0] ≈ −5.1, mq[c1] ≈ +6.7). All three configurations achieve essentially identical task accuracy (99.70–99.75%), so the loss surface offers them as approximately loss-equivalent solutions. The interpretation is that the linear-quad head can in principle exploit sign(m_q) — the quadratic features [‖h_t‖², ‖h_s‖²] are linearly separable by side-of-light-cone — but it lacks the inductive bias to do so reliably; initialization noise selects between qualitatively distinct loss-equivalent solutions rather than between scaled versions of one solution. The MLP nonlinearity in MqMLPHead breaks this near-symmetry by allowing the model to learn a hard threshold on sign(m_q), which makes the canonical sign-split solution strictly preferred under the loss.
[end R3]
...
A single-seed λ-sweep dry run selects the smallest λ_scale that satisfies both mean(|m_q|) ≥ 0.85·target and p10(|m_q|) ≥ 0.5·target_margin on a held-out epoch. We find λ_scale = 0.5 suffices; the resulting calibrated model has in-distribution mean|m_q| = 49.9 and p10 = 46.8 (3-seed mean of GPU run), confirming the calibration takes effect on the whole distribution rather than via a few outliers.
...
[REVISED R1 — replaces the next two paragraphs]
The result is striking. Forcing the Lorentzian backbone to operate at Euclidean's natural |m_q| scale completely destroys the strong sign-aware encoding: 0/24 strong sign-split (vs 11/24 uncalibrated), and sample-level sign accuracy collapses to exactly 0.500 — the chance value, indicating that the model can no longer predict an individual sample's class from sign(m_q) at all. OOD mq-AUC degrades from 0.904 to 0.554. We note that 0.554 is marginally above chance, not below it; the residual 0.054 reflects the head's ability to use small same-side magnitude differences (mq[c0] vs mq[c1] separated by ~1 in absolute value at the calibrated scale). It is not a sign-aware signal. At the same time, in-distribution classification accuracy stays at 99.95%, so the model is still solving the task — it has just stopped using sign(m_q) to do so.
The framing-level point: the diagnostic that fails under calibration is sgnAcc (0.808 → 0.500, a 31-point drop to chance) and the strong sign-split count (11 → 0). mq-AUC degrades but does not invert. The geometric content the calibration was designed to preserve — the encoding's relationship to the literal m_q = 0 light-cone — is what gets destroyed.
Inspection of the training trajectories clarifies the mechanism. Calibrated Lorentzian seed 42 spends epochs 1–3 with both class means in the negative (timelike) region (mq[c0] = −10.35, mq[c1] = −9.64 initially), and they simply both get pushed toward mq ≈ −50 as the calibration loss kicks in, retaining a gap of about 1.0. Calibrated seeds 43 and 44 both stay on the negative side throughout training, ending with mq[c0] ≈ −52, mq[c1] ≈ −48 (gap +4); under sgnAcc this is "all samples are timelike, classification works only by within-cone magnitude," which the metric correctly scores as 0.500. The MqMLPHead exploits the small (but nonzero) magnitude difference between the two same-side clusters to maintain task accuracy, but the geometric content of "which side of the light-cone" is gone. These trajectories are unstable in a way the uncalibrated runs are not.
[end R1]
...
Under sample-level OOD metrics, the Lorentzian and Euclidean backbones are indistinguishable: they reach the same strong sign-split rate (11/24), the same mq-AUC (≈ 0.90), and the same task accuracy. The only persistent architectural difference is encoding scale—Lorentzian's |m_q| ≈ 5 vs Euclidean's ≈ 60. [REVISED R1] A direct calibration test shows that this scale difference is not incidental: forcing the Lorentzian backbone to operate at Euclidean-like scale destroys its sign-aware encoding entirely (0/24 strong sign-split, sample-level sign accuracy collapses from 0.808 to 0.500/chance, OOD mq-AUC degrades from 0.904 to 0.554), suggesting that the two backbones reach equivalent OOD performance through structurally distinct mechanisms. The Lorentzian backbone's sign-aware encoding is tied to its small natural |m_q| scale, which is in turn enforced by SplitNorm's structural prior; it cannot be transplanted to a larger scale by a simple loss adjustment.
...
All code is available at [TODO: URL]. The complete experimental pipeline (battery + OOD evaluation) runs in approximately 11 minutes on a single Colab T4 GPU (single-file driver: paper_reproduce_all_standalone.py, 3 seeds, all 7 sections). We provide the full hyperparameter list, dataset generation code, and per-seed result logs in the supplementary material.
